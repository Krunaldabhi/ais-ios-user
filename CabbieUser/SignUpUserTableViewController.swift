...
class SignUpUserTableViewController: UITableViewController {
    ...
    private var accountKit : AccountKitManager?
    ...
    override func viewDidLoad() {
        ...
        nc.removeObserver(self, name: Notification.Name("otpback"), object: nil)
        nc.addObserver(self, selector: #selector(otpclose), name: Notification.Name("otpback"), object: nil)
    }
    @objc func otpclose(notfication: NSNotification) {
         print("xxx")
        self.presenter?.post(api: .signUp, data: self.userInfo?.toData())
                //self.userInfo?.mobile = Int(phoneNumber ?? "")
    }
    ...
    override func viewDidAppear(_ animated: Bool) {
       ...
       // self.changeNextButtonFrame()
        self.nextImage.image = UIImage.init(named: "forward_White")
...
//        let akPhone = PhoneNumber(countryCode: countryCode ?? "", phoneNumber: phoneNumber)
...
//
//        let fullnumber =  "\(self.countryText.text!)\(mobile)"
//
//
//        let configuration = Configuration(requestCode: { mobile, completion in
//
//                   PhoneAuthProvider.provider().verifyPhoneNumber(mobile, uiDelegate: nil, completion: nil)
//
...
//               })
//
//            //configuration.str_SetPhoneNumber = String(mobile)
//               let vc = PhoneVerificationController(configuration: configuration)
//               vc.delegate = self
//
//               self.present(vc, animated: true)
        
         //PhoneAuthProvider.provider().verifyPhoneNumber(String(mobile), uiDelegate: nil, completion: nil)
        let fullnumber =  "\(self.countryText.text!)\(mobile)"
        var veriID = String()
        ...
        PhoneAuthProvider.provider().verifyPhoneNumber(fullnumber, uiDelegate: nil) { (verificationID, error) in
          if let error = error {
            ...
            let vc = self.storyboard?.instantiateViewController(withIdentifier: "otpverfiyVC") as! otpverfiyVC
             vc.userInfoTmp = self.userInfo
            vc.str_Verification = verificationID
...
extension SignUpUserTableViewController: PhoneVerificationDelegate {
