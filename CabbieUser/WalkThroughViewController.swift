...
extension WalkThroughViewController : UIPageViewControllerDataSource, UIPageViewControllerDelegate {
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewController: UIViewController) -> UIViewController? {
        guard let index = walkThroughControllers.firstIndex(of: viewController), index > 0 else {
            return nil
        }
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerAfter viewController: UIViewController) -> UIViewController? {
        ...
        guard let index = walkThroughControllers.firstIndex(of: viewController), index<self.walkThroughControllers.count-1 else {
            return nil
        }
        print(#function,index)
        return walkThroughControllers[index+1]
