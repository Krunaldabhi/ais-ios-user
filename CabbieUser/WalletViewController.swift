...
//        self.presenter?.get(api: .getCards, parameters: nil)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.isWalletAvailable = User.main.isCardAllowed || User.main.isTelrAllowed
        self.setWalletBalance()
        self.presenter?.get(api: .getProfile, parameters: nil)
        
        // Check if came back to this screen
        // After completing transaction from Telr
        if let telrTransactionId = UserDefaults.standard.value(forKey: Keys.list.telr_transaction_id) as? String {
            // Complete payment flow
            var requestObj = AddTelrMoneyEntity()
            requestObj.amount = self.textFieldAmount.text
            requestObj.payment_mode = PaymentType.TELR.rawValue.lowercased()
            requestObj.user_type = UserType.user.rawValue
            requestObj.transaction_response = telrTransactionId
            self.presenter?.post(api: Base.addMoney, data: requestObj.toData())
            UserDefaults.standard.set(nil, forKey: Keys.list.telr_transaction_id)
...
extension WalletViewController {
    ...
    private func setCardDetails() {
        ...
        } else if let lastFour = UserDefaults.standard.value(forKey: Keys.list.cardLast4Digits) as? String {
           self.labelCard.text = "XXXX-XXXX-XXXX-"+lastFour
        } else {
            self.labelCard.text = Constants.string.telr
        }
    }
...
//        }
        ...
        if self.labelCard.text == PaymentType.TELR.rawValue || (self.labelCard.text?.contains("XXXX-XXXX") ?? false) || self.labelCard.text == Constants.string.telr {
            self.presenter?.post(api: Base.telr, data: nil)
        } else if self.labelCard.text == PaymentType.BRAINTREE.rawValue  {
    ...
    @IBAction private func buttonChangeCardAction() {
        ...
        if let vc = self.storyboard?.instantiateViewController(withIdentifier: Storyboard.Ids.PaymentViewController) as? PaymentViewController{
            ...
            vc.onclickPayment = { type, cardEntity in
                ...
                else if type == PaymentType.TELR {
                    self.labelCard.text = PaymentType.TELR.rawValue
                }
                else {
...
extension WalletViewController : PostViewProtocol {
    ...
    func openTelr(api: Base, data: TelrTransactionEntity?) {
        DispatchQueue.main.async {
            self.loader.isHidden = true
        }
        guard let amount = self.textFieldAmount.text else { return }
        // Open Telr here
        if let transactionId = data?.transaction_id {
            paymentRequest = TelrHelper.preparePaymentRequest(forAmount:amount, withTransactionId: transactionId)
        } else {
            paymentRequest = TelrHelper.preparePaymentRequest(forAmount:amount)
        }
        ...
        self.present(telrVC, animated: true, completion: nil)
