...
extension PaymentViewController {
    ...
    private func validatePaymentModes() {
        ...
        // Add telr payment option here @hardik
        let isTelrRow = (!User.main.isTelrAllowed) ? 0 : 1
        totalCount.updateValue(isTelrRow, forKey: telrSection)
    ...
    private func swipeAction(at indexPath : IndexPath) -> UISwipeActionsConfiguration?{
        ...
        guard indexPath.section == 5 else { return nil}
...
extension PaymentViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let tableCell = tableView.dequeueReusableCell(withIdentifier: self.tableCellId, for: indexPath) as? PaymentCell {
            ...
            if indexPath.section == 0  {
                ...
                if let last4Digits = UserDefaults.standard.value(forKey: Keys.list.cardLast4Digits) as? String {
                    tableCell.labelPayment.text = "XXXX-XXXX-XXXX-"+String.removeNil(last4Digits)
                } else {
                    tableCell.labelPayment.text = Constants.string.telr.localize()
                }
                if paymentTypeStr == PaymentType.TELR.rawValue {
                    tableCell.accessoryType = .checkmark
                }
    ...
    override func tableView(_ tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat {
        return 0.01//UITableView.automaticDimension
    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.dismiss(animated: true) {
            ...
            else if indexPath.section == 3, self.cardsList.count > indexPath.row {
                self.onclickPayment?(.CARD ,self.cardsList[indexPath.row])
            }
            else if indexPath.section == 2 {
                self.onclickPayment?(.TELR, nil)
            }
            else {
