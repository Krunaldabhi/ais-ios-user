...
extension HomeViewController {
    ...
    func showRideNowView(with source : [Service]) {
        ...
        if self.rideNowView == nil {
            ...
            self.rideNowView?.frame = CGRect(origin: CGPoint(x: 0, y: self.view.frame.height-(self.rideNowView!.frame.height+30)), size: CGSize(width: self.view.frame.width, height: self.rideNowView!.frame.height+30))
...
//                self?.showEstimationView(with: service)
            ...
            self.rideNowView?.onClickService = { [weak self] service in
                guard let self = self, self.listOfProviders != nil, service != nil else {return}
    ...
    func showEstimationView(with service : Service, paymentType: PaymentType){
    ...
    func showPaymentOptionDialog(with service : Service) {
        let alert = PopupDialog(title: "Select Payment Method", message: nil)
        let okButton =  PopupDialogButton(title: PaymentType.CASH.rawValue, action: { [weak self] in
            // Cash selected
            self?.showEstimationView(with: service, paymentType: .CASH)
            alert.dismiss()
        })
        let cancelButton =  PopupDialogButton(title: PaymentType.CARD.rawValue, action: { [weak self] in
            // Card selected
            self?.showEstimationView(with: service, paymentType: .TELR)
            alert.dismiss()
        })
        ...
        if self.invoiceView == nil, let invoice = Bundle.main.loadNibNamed(XIB.Names.InvoiceView, owner: self, options: [:])?.first as? InvoiceView {
            ...
            self.invoiceView?.onClickPaynow = { tipsAmount in
                ...
                if tipsAmount>0 {
                    ...
                    request.tips = requestObj.tips
                }
                if self.invoiceView?.paymentType == .TELR {
                    requestObj.payment_mode = .TELR
                    self.currentRequest = request
                    self.presenter?.post(api: .telr, data: nil)
//                    self.presenter?.post(api: .payNow, data: requestObj.toData())
                }
                else if self.invoiceView?.paymentType == .BRAINTREE {
    ...
    func handle(request : Request) {
        ...
        case .completed:
            ...
            //            self.buttonWithoutDest.isHidden = false
            riderStatus = .none
            if request.payment_mode == .CARD || request.payment_mode == .TELR {
