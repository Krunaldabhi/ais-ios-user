...
class HomeViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let telrTransactionId = UserDefaults.standard.value(forKey: Keys.list.telr_transaction_id) as? String, let request = self.currentRequest {
            // Complete payment flow
            var paymentRequest = AddTelrMoneyEntity()
            paymentRequest.transaction_response = telrTransactionId
            paymentRequest.request_id = request.id
            paymentRequest.payment_mode = PaymentType.TELR.rawValue
            paymentRequest.tips = request.tips
            self.presenter?.post(api: .payNow, data: paymentRequest.toData())
...
extension HomeViewController {
    ...
    func showRideNowWithoutDest(with source : [Service]) {
        ...
        if self.rideNowView == nil {
            ...
            self.rideNowView?.frame = CGRect(origin: CGPoint(x: 0, y: self.view.frame.height-(self.rideNowView!.frame.height+10)), size: CGSize(width: self.view.frame.width, height: self.rideNowView!.frame.height+10))
            ...
            self.rideNowView?.onClickProceed = { [weak self] service in
                ...
                // Show cash or card screen
                self?.showPaymentOptionDialog(with: service)
            }
...
extension HomeViewController : PostViewProtocol {
    ...
    func openTelr(api: Base, data: TelrTransactionEntity?) {
        guard let request = self.currentRequest, let amount = request.payment?.payable else { return }
        let tips = request.tips ?? 0
        var paymentRequest:PaymentRequest
        // Open Telr here
        if let transactionId = data?.transaction_id {
            paymentRequest = TelrHelper.preparePaymentRequest(forAmount:"\(amount+tips)", withTransactionId: transactionId)
        } else {
            paymentRequest = TelrHelper.preparePaymentRequest(forAmount:"\(amount+tips)")
        }
        ...
        self.navigationController?.pushViewController(telrVC, animated: true)
